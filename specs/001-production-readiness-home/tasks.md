...
- [ ] T001 [US4] Configure Vercel deployment for the Deno app (runtime, static serving from `public/`, env/secrets); document in `README.md`; park the Deno Deploy CI + Fly config. — `vercel.json`, `.github/workflows/deno.yml`, `Dockerfile`, `fly.toml` (D1-17) — **highest risk, do first**
...
- [ ] T003 [P] [US1] Add email-delivery module (provider client + formatted booking template). — `server/email.ts` (D1-10)

## Phase 2: US1 — Book the band (Priority: P1) 🎯 MVP

- [ ] T004 [US1] Rework booking form fields → First Name*, Last Name*, Email*, Phone(optional), Event Date*, Event Type, Location/Venue*, Budget, Message*. — `public/index.html` (D1-09, FR-001)
...
- [ ] T006 [US1] Server-side validation of required fields + email-failure handling (surface error, don't lose the request). — `server/main.ts` (FR-003, CHK001/CHK022)
- [ ] T007 [US1] Input sanitization so field content cannot inject into the outbound email. — `server/email.ts` (CHK024)
- [ ] T008 [US1] Spam protection: Cloudflare Turnstile + honeypot + server-side rate limit. — `public/index.html`, `public/app.js`, `server/main.ts` (D1-12, FR-004)
- [ ] T009 ⛔ [US1] Set the real destination booking email (env var). — config (D1-11, FR-005)
...
- [ ] T019 [P] [US4] SEO: favicon + Open Graph/Twitter meta + share preview image. — `public/index.html`, `public/assets/` (D1-18, FR-017)
- [ ] T020 [P] [US4] Install privacy-friendly analytics (Umami or Plausible; no cookie banner). — `public/index.html` (D1-19, FR-018)
- [ ] T021 [US4] Privacy policy page + consent line on the form (retains First/Last Name, Email, Phone). — `public/privacy.html`, `public/index.html` (D1-20, FR-019)
- [ ] T022 [US4] Mobile responsive pass (hero scaling, 2-col grids, deck/nav) — no overflow at 360–414px. — `public/styles.css`, `public/index.html` (D1-21, FR-020)
- [ ] T023 [P] [US4] Themed 404 page ("SIDE B NOT FOUND — rewind ◂"). — `public/404.html`, `server/main.ts` (D1-22, FR-021)
...
- [ ] T026 Hide the Gigs panel (keep code/data to re-enable). — `public/index.html`, `public/app.js` (D2-01)
...
- Tasks marked ⛔ are blocked on owner input (email, YouTube URLs, socials, About bio, EPK assets, final copy).
